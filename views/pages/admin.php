<?php
/**
 * Dashboard admin (inclus depuis admin.php racine et api/admin.php).
 */
...
<body>
    ...
    <main class="admin-page">
        <div class="admin-container">
            <div class="admin-header">
                ...
                <a href="/auth?action=logout" class="btn-logout-nav" style="background:#ef4444; color:white; padding: 0.75rem 1.5rem; border-radius:12px; text-decoration:none; font-weight:bold;">Déconnexion</a>
            </div>
    ...
    <script>
        ...
        async function uploadFile(file, textInput, statusId) {
            ...
            try {
                const res = await fetch('/admin_actions?action=upload_file', { method: 'POST', body: formData });
        ...
        async function loadVeille() {
            const data = await apiCall('/admin_actions?action=list_veille');
        ...
        document.getElementById('veilleForm').onsubmit = async (e) => {
            ...
            const res = await apiCall('/admin_actions?action=add_veille', 'POST', body);
            if(res.success) { showStatus('v_status', 'Article ajouté'); e.target.reset(); loadVeille(); }
        }
        async function deleteVeille(id) { if(confirm('Sûr ?')) { await apiCall(`/admin_actions?action=delete_veille&id=${id}`, 'DELETE'); loadVeille(); } }

        // --- USERS ---
        async function loadUsers() {
            const data = await apiCall('/admin_actions?action=list_users');
        ...
        document.getElementById('accessForm').onsubmit = async (e) => {
            ...
            const res = await apiCall('/admin_actions?action=add_user', 'POST', body);
            if(res.success) { showStatus('u_status', 'Accès créé'); e.target.reset(); loadUsers(); }
        }
        async function deleteUser(id) { if(confirm('Sûr ?')) { await apiCall(`/admin_actions?action=delete_user&id=${id}`, 'DELETE'); loadUsers(); } }

        // --- FORMATIONS ---
        async function loadFormations() {
            const data = await apiCall('/admin_actions?action=list_formations');
        ...
        async function deleteFormation(id) { if(confirm('Sûr ?')) { await apiCall(`/admin_actions?action=delete_formation&id=${id}`, 'DELETE'); loadFormations(); } }

        // --- CERTIFICATIONS ---
        async function loadCerts() {
            const data = await apiCall('/admin_actions?action=list_certifications');
        ...
        document.getElementById('certForm').onsubmit = async (e) => {
            ...
            const res = await apiCall('/admin_actions?action=add_certification', 'POST', body);
            if(res.success) { showStatus('c_status', 'Certification ajoutée'); e.target.reset(); loadCerts(); }
        }
        async function deleteCert(id) { if(confirm('Sûr ?')) { await apiCall(`/admin_actions?action=delete_certification&id=${id}`, 'DELETE'); loadCerts(); } }
